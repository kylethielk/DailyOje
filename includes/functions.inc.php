...
function root_url()
{
    ...
    return $protocol . "://" . $_SERVER['HTTP_HOST'] . $port . WEB_ROOT;
}
...
/**
 * Remove a parameter from a string
 *
 * remove_get_parameter("http://www.myurl.com/?p1=d&p2=k","p1") => "http://www.myurl.com/?p2=k"
 * @param $url
 * @param $parameter_name
 * @return mixed
 */
function strip_url_parameter($url, $parameter_name)
{
    $url = preg_replace('/([?&])' . $parameter_name . '=[^&]+(&|$)/', '$1', $url);
...
function parse_boolean($val)
{
    ...
    if (strtolower($val) == "t" || strtolower($val) == "true" || $val === true)
    {
        return true;
    }
    return false;
