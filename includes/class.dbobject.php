...
class DBObject
{
    ...
    /**
     * Must set count result as 'cnt' in SQL.
     *
     * SELECT COUNT(*) as cnt
     * @param $sqlString
     * @return mixed
     */
    public function countFromSql($sqlString)
    {
        $db = Database::getDatabase();
    ...
    public function count($columns = null)
    {
        ...
        $query = "SELECT COUNT({$this->idColumnName}) as cnt FROM {$this->tableName} ";
    ...
    /**
     * Select a single result from the database based on one or more columns.
     * @param array $columns Associative array with key == columnName and value equal to the value we are searching on.
     * @return bool True If result is found.
     */
    public function selectWithMultipleColumns($columns = array())
    {
        ...
        $query .= implode(' AND ', $queries) . " LIMIT 1";
